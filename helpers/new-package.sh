...
HASHICORP_IGNORED='(terraform-provider|driver|plugin|vagrant|consul-|docker|helper|atlas-)'
OS="${OS:-"linux"}"
ARCH="${ARCH:-"amd64"}"
...
if [ "$packageType" = "bin" ] || [ "$packageType" = "tarball" ]; then
...
if [ -d ${VENDORPATH}/${APP} ]; then
  ...
  echo ""
  echo "Then run it from here:"
  echo "  ${INSTALL_PATH}/${APP}"
  echo ""
#   echo "Test your package apk build with the following:"
#   echo "  make -C ${PACKAGES_PATH} docker/build/apk/shell"
